...
(async () => {
  ...
  try {
    ...
    // A one-item review batch continues directly into another puzzle.
    ...
    await page.waitForFunction(id=>play.card.id!==id && play.awaiting,firstId);
    assert.equal(await page.locator('#sumTitle').count(),0);
    await page.evaluate(()=>go('puzzles'));
    await page.locator('#puzzleTheme').selectOption('lichess-mateIn1');
    await page.locator('#puzzleLevel').selectOption('base');
    const expected=await page.locator('.puzzle-row .cl-top b').nth(1).innerText();
    await page.locator('.puzzle-row').first().click();
    await solve();
    assert.equal(await page.locator('.train-info button:visible').first().getAttribute('id'),'nextPuzzle');
    await page.locator('#nextPuzzle').click();
    await page.waitForFunction(name=>play.card.name===name,expected);
    assert.equal(await page.locator('#hintBtn').isVisible(),true);
    await solve();
    await page.locator('#finishPuzzles').click();
    ...
    console.log('PASS: 200 unique puzzles; category/level filters; hidden mixed themes; hint review; clean retry; multi-move solution; offline catalogue and saved progress; opening lesson regression; sequential filtered catalogue; continuous batches; next button first.');
