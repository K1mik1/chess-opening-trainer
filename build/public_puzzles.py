...
BANDS = [(500, 1000, 10), (1000, 1400, 10), (1400, 1801, 5)]
    ...
    with urlopen(URL, timeout=60) as response:
        with zstandard.ZstdDecompressor().stream_reader(response) as stream:
            for row in csv.DictReader(io.TextIOWrapper(stream)):
                ...
                if len(seen) % 25 == 0:
                    print(f'Selezionati {len(seen)}/200', flush=True)
                if len(seen) == 200:
...
def main():
    selected = json.loads(SOURCE.read_text()) if SOURCE.exists() else select()
    packs = []
    for category, (theme, name, blurb) in zip(selected, CATEGORIES):
        ...
        assert len(items) == 25 and all(items)
    ...
    print('200 problemi verificati, 8 categorie, 3 fasce di difficoltà.')
